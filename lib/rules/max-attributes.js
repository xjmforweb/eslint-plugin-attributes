...
module.exports = {
    ...
    create: function(context) {
        ...
        const attrStrLimit = context.options.length && context.options[0].attrStrLimit || 40
                    ...
                    if (node.startTag.attributes.length > max) {
                        ...
                        if (node.startTag.loc.start.line !== node.startTag.loc.end.line) {
                            ...
                            context.report({
                                ...
                                data: {},
                                fix: fixer => {
                                    // tag内没有内容，只有换行。自闭的tag不处理
                                    if (node.children.length === 1 && !node.startTag.selfClosing) {
